...
import static cutom.rmi.networking.LoopbackSslSocketFactory.followRedirect;
...
public class ProxyClient implements JConsoleContext {
    ...
    private ProxyClient(String hostName, int port,
        ...
        if (hostName.equals("localhost") && port == 0) {
            ...
            String boundName = list[0];
//            final String urlPath = "/jndi/rmi://" + hostName + ":" + port + "/jmxrmi";
            Logger.println_bl(String.format("Binded Object %s\t%s\t%s\t", hostName,port,s));
            final String urlPath =  "/jndi/rmi://" + hostName + ":" + port + "/" + boundName;
            Logger.println_bl("Connection  url : " + urlPath);
    ...
    private void tryConnect(boolean requireRemoteSSL) throws IOException {
        if (jmxUrl == null && "localhost".equals(hostName) && port == 0) {
            ...
            */
            ...
            RMISocketFactory my = new LoopbackSocketFactory(this.registryHostName, fac, followRedirect);
            ...
            LoopbackSslSocketFactory.host = this.registryHostName;
